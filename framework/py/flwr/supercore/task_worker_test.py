...
# ==============================================================================
"""Tests for the shared single-use prestarted task worker."""
...
from . import task_worker, task_worker_protocol
...
class _TestInvocation:
    """Describe one invocation used to exercise the shared worker."""
    ...
    @classmethod
    def from_payload(cls, payload: object) -> _TestInvocation:
        """Validate and construct one test invocation."""
        token, runtime_api_address, insecure, root_certificates_path = (
            task_worker.parse_invocation_payload(payload, "Test")
        )
        return cls(
            token=token,
    ...
    thread = threading.Thread(
        target=lambda: result.append(
            task_worker._serve_connection(
                server, run_once, _TestInvocation.from_payload, "Test"
            )
        )
    )
    thread.start()
...
def test_protocol_rejects_invalid_messages() -> None:
    ...
    with pytest.raises(ValueError, match="non-empty string"):
        _TestInvocation.from_payload(_invocation(token=""))
    with pytest.raises(ValueError, match="unexpected fields"):
        _TestInvocation.from_payload(_invocation(extra=True))
...
def test_worker_cleans_up_socket_and_markers(
    ...
    assert (
        task_worker.serve_prestarted_worker(
            socket_path,
            ready_file,
            busy_file,
            Mock(),
            _TestInvocation.from_payload,
            "Test",
        )
        == 0
    )
...
def test_dispatch_relays_accepted_output(
    ...
    assert (
        task_worker.dispatch_prestarted_task(
            _invocation(), Path("/tmp/test-worker.sock"), "Test"
        )
        == 0
    )
...
def test_worker_rejects_failure_before_acceptance() -> None:
    ...
    assert frames == [
        {
            "event": "rejected",
            "reason": "Prestarted Test worker could not start task.",
        }
    ]
...
def test_worker_reports_graceful_exit_after_acceptance() -> None:
    ...
    with pytest.raises(SystemExit):
        task_worker._serve_channel(channel, stop, _TestInvocation.from_payload, "Test")
...
def test_disconnect_does_not_abort_accepted_task() -> None:
    ...
    thread = threading.Thread(
        target=lambda: result.append(
            task_worker._serve_connection(
                server, run_once_mock, _TestInvocation.from_payload, "Test"
            )
        )
    )
    thread.start()
...
def test_saturated_output_channel_does_not_abort_task() -> None:
    ...
    result = task_worker._serve_channel(
        channel, run_once, _TestInvocation.from_payload, "Test"
    )
